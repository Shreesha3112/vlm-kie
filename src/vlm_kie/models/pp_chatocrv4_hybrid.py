"""PP-ChatOCRv4-Hybrid — PaddleOCR OCR + Qwen2.5 (transformers) for structured extraction.

NOTE: This is NOT the real PP-ChatOCRv4 pipeline. It is a custom hybrid that uses
PaddleOCR for text extraction and a local Qwen2.5 LLM (transformers) for structuring.
The real PP-ChatOCRv4 pipeline (PPChatOCRv4Doc with RAG + MLLM + LLM) is in pp_chatocrv4.py.
"""
...
class PPChatOCRv4HybridBackend(BaseVLM):
    """Hybrid pipeline:
